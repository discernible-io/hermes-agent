...
cmd_start_pod() {
  ...
  # nginx owns HERMES_INGRESS_PORT (80 for Telegram). Adapter must listen elsewhere.
...
cmd_status() {
  ...
  if hermes_is_pod_mode || podman pod exists "${HERMES_POD:-hermes-agent-pod}" 2>/dev/null; then
    ...
    if [[ -n "${HERMES_PUBLIC_HOST:-}" ]]; then
      echo "Ingress:  https://${HERMES_PUBLIC_HOST}:${HERMES_INGRESS_PORT:-80}/health"
      echo "Webhook:  https://${HERMES_PUBLIC_HOST}:${HERMES_INGRESS_PORT:-80}/webhooks/<route>"
      echo "Telegram: https://${HERMES_PUBLIC_HOST}:${HERMES_INGRESS_PORT:-80}/telegram"
    fi
