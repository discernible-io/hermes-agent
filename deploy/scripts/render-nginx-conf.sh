...
#   HERMES_INGRESS_PORT  default 11443
#   WEBHOOK_PORT         upstream port (default 8644, pod-local)
set -euo pipefail
...
ingress_port="${HERMES_INGRESS_PORT:-11443}"
...
http {
    ...
    # Hermes — webhooks @ ${host}:${ingress_port}
    server {
...
echo "Rendered ${out} (host=${host}, ingress=${ingress_port}, webhook_upstream=${webhook_port})"
